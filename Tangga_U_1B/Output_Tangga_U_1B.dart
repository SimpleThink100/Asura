...
class Output_Tangga_U_1BPage extends StatefulWidget {

  final Projek projek;
  final Task task;
  Output_Tangga_U_1BPage({this.projek,this.task});

  @override
  State<Output_Tangga_U_1BPage> createState() => _Output_Tangga_U_1BPageState();
}

class _Output_Tangga_U_1BPageState extends State<Output_Tangga_U_1BPage> {
    ...
    return InkWell(
      onTap: () {
        switch (simbol) {
          case 1:
            if (widget.task.kD == 1) {
              Get.to(OutputD_Tangga_U_1BPage(projek: widget.projek,task: widget.task,));
            }
            break;
          case 2:
            if (widget.task.kL == 1) {
              Get.to(OutputA_Tangga_U_1BPage(projek: widget.projek,task: widget.task,));
            }
            break;
          case 3:
            if (widget.task.kM == 1) {
              Get.to(OutputG_Tangga_U_1BPage(projek: widget.projek,task: widget.task,));
            }
            break;
          case 4:
            if (widget.task.kM == 1) {
              Get.to(OutputP_Tangga_U_1BPage(projek: widget.projek,task: widget.task,));
            }
            break;
          case 5:
            if (widget.task.kO == 1) {
              print('export pdf');
              // Get.to(Output_Tangga_U_1BPage(projek: widget.projek,task: widget.task,));
            }
            break;
          default:
            return print('0');
