...
class Gambar_Tangga_L_1 extends CustomPainter {
  final c = (1.0).toT_L_a();
  @override
  void paint(Canvas canvas, Size size) {
    final double JB=50;
    final double JB2 = JB*2 + (TinggiT_1 + TinggiT_2)*c;
    ...
    final double xT_2=TebalPelatTanggaPilih_2/sina_2-(TebalPelatBordesPilih_2)*cosa_2/sina_2;
    final double yT_2=TebalPelatTanggaPilih_2/sina_2+(Optrede_2-TebalPelatBordesPilih_2)*cosa_2/sina_2;
    ...
    //gambar tangga atas
    var path1 = Path();
    path1.moveTo((JA).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_1 - TinggiT_2)*c);
    path1.lineTo((JA).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_1)*c);
    path1.lineTo((JA).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_1+yB_1)*c);
    path1.lineTo((JA+(LebarT+Antrede_1*_teratas-yT_1)*c).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_1+yB_1)*c);
    path1.lineTo((JA+(LebarT+PanjangT_1-xT_1)*c).toMirror(M, MV), TinggiCanvasa - JB2 + (0.0)*c);
    path1.lineTo((JA+(LebarT+PanjangT_1)*c).toMirror(M, MV), TinggiCanvasa - JB2 + (0.0)*c);
    for (var i = 0; i <= OptredeNT_1-1; i++) {
      path1.lineTo((JA+(LebarT+PanjangT_1-Antrede_1*(i))*c).toMirror(M, MV),
          TinggiCanvasa - JB2 + (-Optrede_1*i)*c);
      path1.lineTo((JA+(LebarT+PanjangT_1-Antrede_1*(i))*c).toMirror(M, MV),
          TinggiCanvasa - JB2 + (-Optrede_1*(i+1))*c);
    }
    path1.lineTo((JA + (LebarT )*c).toMirror(M, MV),
        TinggiCanvasa - JB2+(-TinggiT_1)*c);
    path1.lineTo((JA + (LebarT )*c).toMirror(M, MV),
        TinggiCanvasa - JB2+(-TinggiT_1-TinggiT_2)*c);
    canvas.drawPath(path1, Line3);

    canvas.drawLine(
        Offset((JA+(0.0)*c).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_1 - TinggiT_2)*c),
        Offset((JA +(0.0)*c).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_1)*c),
        Line1);
    canvas.drawLine(
        Offset((JA+(0.0)*c).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_1)*c),
        Offset((JA +(0.0)*c).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_1+yB_1)*c),
        Line1);
    canvas.drawLine(
        Offset((JA +(0.0)*c).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_1+yB_1)*c),
        Offset((JA +(LebarT+Antrede_1*_teratas-yT_1)*c).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_1+yB_1)*c),
        Line1);
    canvas.drawLine(
        Offset((JA +(LebarT+Antrede_1*_teratas-yT_1)*c).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_1+yB_1)*c),
        Offset((JA +(LebarT+PanjangT_1-xT_1)*c).toMirror(M, MV), TinggiCanvasa - JB2 + (0.0)*c),
        Line1);
    canvas.drawLine(
        Offset((JA +(LebarT+PanjangT_1-xT_1)*c).toMirror(M, MV), TinggiCanvasa - JB2 + (0.0)*c),
        Offset((JA +(LebarT+PanjangT_1)*c).toMirror(M, MV), TinggiCanvasa - JB2 + (0.0)*c),
        Line1);

    for (var i = 0; i <= OptredeNT_1 - 1; i++) {

      canvas.drawLine(
          Offset((JA +(LebarT+PanjangT_1-Antrede_1*i)*c).toMirror(M, MV),
              TinggiCanvasa - JB2 + (-Optrede_1*i)*c),
          Offset((JA +(LebarT+PanjangT_1-Antrede_1*i)*c).toMirror(M, MV),
              TinggiCanvasa - JB2 + (-Optrede_1*(i+1))*c),
          Line1);
      ...
      canvas.drawLine(
          Offset((JA +(LebarT+PanjangT_1-Antrede_1*i)*c).toMirror(M, MV),
              TinggiCanvasa - JB2 + (-Optrede_1*(i+1))*c),
          Offset((JA +(LebarT+PanjangT_1-Antrede_1*(i+1))*c).toMirror(M, MV),
              TinggiCanvasa - JB2 + (-Optrede_1*(i+1))*c),
          Line1);

    }
    canvas.drawLine(
        Offset((JA +(LebarT)*c).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_1)*c),
        Offset((JA +(LebarT)*c).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_1-TinggiT_2)*c),
        Line1);
    for (var i = 0; i <= OptredeNT_2; i++) {
      canvas.drawLine(
          Offset((JA).toMirror(M, MV),
              TinggiCanvasa - JB2 + (-TinggiT_1 - Optrede_2 * (i))*c),
          Offset(
              (JA + (LebarT)*c).toMirror(M, MV),
              TinggiCanvasa - JB2 +
                  (-TinggiT_1 -Optrede_2 * (i))*c),
          Line1);
    }
    //balok
    var path2 = Path();
    path2.moveTo((JA+(-bB/2)*c).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_1)*c);
    path2.lineTo((JA+(-bB/2)*c).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_1+hB)*c);
    path2.lineTo((JA+(LebarT)*c).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_1+hB)*c);
    path2.lineTo((JA+(LebarT)*c).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_1)*c);
    canvas.drawPath(path2, Line3);
    canvas.drawLine(
        Offset((JA+(-bB/2)*c).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_1)*c),
        Offset((JA+(-bB/2)*c).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_1+hB)*c),
        Line1);
    canvas.drawLine(
        Offset((JA+(-bB/2)*c).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_1+hB)*c),
        Offset((JA+(LebarT)*c).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_1+hB)*c),
        Line1);
    canvas.drawLine(
        Offset((JA+(LebarT)*c).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_1+hB)*c),
        Offset((JA+(LebarT)*c).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_1)*c),
    ...
    canvas.drawLine(
        Offset((JA+(0.0)*c).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_1)*c),
        Offset((JA+(0.0)*c).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_1+hB)*c),
        Line1);
    //keterangan11 panjang tangga
    //lebar
    final Kete = Para(Text:LebarT.toStringAsFixed(0) + " cm",Lebar: LebarT*c,Posisi:TextAlign.center);
    canvas.drawParagraph(Kete.toKetW(),
        Offset((JA + (0.0)*c).toMirror(M, MV), TinggiCanvasa - JB + 10));
    canvas.drawParagraph(Kete.toKetW(),
        Offset((JA + (0.0)*c).toMirror(M, MV), TinggiCanvasa - JB2 + 10));
    //panjangT2
    final Kete17 = Para(Text:PanjangT_2.toStringAsFixed(0) + " cm",Lebar: PanjangT_2*c,Posisi:TextAlign.center);
    ...
    canvas.drawParagraph(
        ...
        Offset((JA + (LebarT+PanjangT_2)*c).toMirror(M, MV),
            TinggiCanvasa - JB + 10));
    //panjang T1
    final Kete16 = Para(Text:PanjangT_1.toStringAsFixed(0) + " cm",Lebar: PanjangT_1*c,Posisi:TextAlign.center);
    canvas.drawParagraph(
        Kete16.toKetW(),
        Offset((JA + (LebarT)*c).toMirror(M, MV),
            TinggiCanvasa - JB2 + 10));

    //keterangan15 sudut atas
    final Kete15 = Para(Text:Sudut_2.toStringAsFixed(0) + " °");
    canvas.drawParagraph(
        Kete15.toKetW(),
        Offset(JA + (LebarT + PanjangT_1 * 1 / 10)*c,
            TinggiCanvasa - JB2 - Optrede_1*c));

    //keterangan sudut bawah
    final Kete13 = Para(Text:Sudut_1.toStringAsFixed(0) + " °");
    canvas.drawParagraph(
        Kete13.toKetW(),
        Offset(JA + (LebarT + PanjangT_2 * 2 / 5)*c,
    ...
    path.lineTo((JA + (LebarT + PanjangT_2+yT_2-Antrede_2*_teratas)*c).toMirror(M, MV),
        TinggiCanvasa - JB + (-TinggiT_1 - TinggiT_2 + yB_2)*c);
    path.lineTo((JA + (LebarT+xT_2)*c).toMirror(M, MV),
    ...
    canvas.drawLine(
        Offset((JA + (LebarT + PanjangT_2+PanjangB_2)*c).toMirror(M, MV),
            TinggiCanvasa - JB + (-TinggiT_1 - TinggiT_2)*c),
        Offset((JA + (LebarT + PanjangT_2+PanjangB_2)*c).toMirror(M, MV),
            TinggiCanvasa - JB + (-TinggiT_1 - TinggiT_2 + yB_2)*c), Line1);
    canvas.drawLine(
        Offset((JA + (LebarT + PanjangT_2+PanjangB_2)*c).toMirror(M, MV),
            TinggiCanvasa - JB + (-TinggiT_1 - TinggiT_2+yB_2)*c),
        Offset((JA + (LebarT + PanjangT_2+yT_2-Antrede_2*_teratas)*c).toMirror(M, MV),
            TinggiCanvasa - JB + (-TinggiT_1 - TinggiT_2+yB_2)*c),
        Line1);
    canvas.drawLine(
        Offset((JA + (LebarT + PanjangT_2+yT_2-Antrede_2*_teratas)*c).toMirror(M, MV),
            TinggiCanvasa - JB + (-TinggiT_1 - TinggiT_2 +yB_2)*c),
        Offset((JA + (LebarT+xT_2)*c).toMirror(M, MV),
            TinggiCanvasa - JB + (-TinggiT_1 + yB_1)*c),
        Line1);
    canvas.drawLine(
        Offset((JA + (LebarT)*c).toMirror(M, MV),
    ...
    //balok atas
    var path3 = Path();
    path3.moveTo((JA + (LebarT+PanjangT_2+PanjangB_2+bB/2)*c).toMirror(M, MV),
        TinggiCanvasa - JB+(-TinggiT_1-TinggiT_2+hB)*c);
    path3.lineTo((JA + (LebarT+PanjangT_2+PanjangB_2-bB/2)*c).toMirror(M, MV),
        TinggiCanvasa - JB+(-TinggiT_1-TinggiT_2+hB)*c);
    path3.lineTo((JA + (LebarT+PanjangT_2+PanjangB_2-bB/2)*c).toMirror(M, MV),
        TinggiCanvasa - JB+(-TinggiT_1-TinggiT_2)*c);
    path3.lineTo((JA + (LebarT+PanjangT_2+PanjangB_2+bB/2)*c).toMirror(M, MV),
    ...
    canvas.drawLine(
        Offset((JA + (LebarT+PanjangT_2+PanjangB_2+bB/2)*c).toMirror(M, MV),
            TinggiCanvasa - JB+(-TinggiT_1-TinggiT_2+hB)*c),
        Offset((JA + (LebarT+PanjangT_2+PanjangB_2-bB/2)*c).toMirror(M, MV),
            TinggiCanvasa - JB+(-TinggiT_1-TinggiT_2+hB)*c),
        Line1);
    ...
    canvas.drawLine(
        ...
        Line1);
    //balok bawah
    path3.moveTo((JA + (0.0)*c).toMirror(M, MV),
        TinggiCanvasa - JB+(-TinggiT_1+hB)*c);
    path3.lineTo((JA + (-bB/2)*c).toMirror(M, MV),
        TinggiCanvasa - JB+(-TinggiT_1+hB)*c);
    path3.lineTo((JA + (-bB/2)*c).toMirror(M, MV),
        TinggiCanvasa - JB+(-TinggiT_1)*c);
    path3.lineTo((JA + (0.0)*c).toMirror(M, MV),
        TinggiCanvasa - JB+(-TinggiT_1)*c);
    canvas.drawPath(path3, Line3);
    canvas.drawLine(
        Offset((JA + (0.0)*c).toMirror(M, MV),
            TinggiCanvasa - JB+(-TinggiT_1+hB)*c),
        Offset((JA + (-bB/2)*c).toMirror(M, MV),
            TinggiCanvasa - JB+(-TinggiT_1+hB)*c),
        Line1);
    canvas.drawLine(
        Offset((JA + (-bB/2)*c).toMirror(M, MV),
    ...
    //garis keterangan panjang bawah
    canvas.drawLine(Offset((JA).toMirror(M, MV), TinggiCanvasa - JB + JK),
        Offset((JA + LebarT*c).toMirror(M, MV), TinggiCanvasa - JB + JK), Line1);
    canvas.drawLine(Offset((JA + LebarT*c).toMirror(M, MV), TinggiCanvasa - JB + JK - 5),
        Offset((JA + LebarT*c).toMirror(M, MV), TinggiCanvasa - JB + JK + 5), Line1);
    canvas.drawLine(
        Offset((JA + LebarT*c).toMirror(M, MV), TinggiCanvasa - JB + JK),
        Offset((JA + (LebarT + PanjangT_2)*c).toMirror(M, MV), TinggiCanvasa - JB + JK),
        Line1);
    canvas.drawLine(
        Offset((JA + (LebarT+PanjangT_2)*c).toMirror(M, MV), TinggiCanvasa - JB + JK-5),
        Offset((JA + (LebarT + PanjangT_2)*c).toMirror(M, MV), TinggiCanvasa - JB + JK+5),
        Line1);
    canvas.drawLine(
        Offset((JA + (LebarT+PanjangT_2)*c).toMirror(M, MV), TinggiCanvasa - JB + JK),
        Offset((JA + (LebarT + PanjangT_2+PanjangB_2)*c).toMirror(M, MV), TinggiCanvasa - JB + JK),
        Line1);
    //atas
    canvas.drawLine(Offset((JA).toMirror(M, MV), TinggiCanvasa - JB2 + JK),
        Offset((JA + LebarT*c).toMirror(M, MV), TinggiCanvasa - JB2 + JK), Line1);
    canvas.drawLine(Offset((JA + LebarT*c).toMirror(M, MV), TinggiCanvasa - JB2 + JK - 5),
        Offset((JA + LebarT*c).toMirror(M, MV), TinggiCanvasa - JB2 + JK + 5), Line1);
    canvas.drawLine(
        Offset((JA + LebarT*c).toMirror(M, MV), TinggiCanvasa - JB2 + JK),
        Offset((JA + (LebarT + PanjangT_1)*c).toMirror(M, MV), TinggiCanvasa - JB2 + JK),
        Line1);

    //garis keterangan kanan bawah
    canvas.drawLine(
        Offset(JA + ((PanjangT_1 > PanjangT_2 ? PanjangT_1 : PanjangT_2) + LebarT+PanjangB_2)
            *c + JK, TinggiCanvasa - JB),
        Offset(JA + ((PanjangT_1 > PanjangT_2 ? PanjangT_1 : PanjangT_2) + LebarT+PanjangB_2)
            *c + JK, TinggiCanvasa - JB - TinggiT_1*c),
        Line1);
    canvas.drawLine(
        Offset(JA + ((PanjangT_1 > PanjangT_2 ? PanjangT_1 : PanjangT_2) + LebarT+PanjangB_2)
            *c + JK + 5,
    ...
    //atas
    canvas.drawLine(
        Offset(JA + ((PanjangT_1 > PanjangT_2 ? PanjangT_1 : PanjangT_2) + LebarT)
            *c + JK, TinggiCanvasa - JB2),
        Offset(JA + ((PanjangT_1 > PanjangT_2 ? PanjangT_1 : PanjangT_2) + LebarT)
            *c + JK, TinggiCanvasa - JB2 - TinggiT_1*c),
        Line1);
    canvas.drawLine(
        Offset(JA + ((PanjangT_1 > PanjangT_2 ? PanjangT_1 : PanjangT_2) + LebarT)
            *c + JK + 5, TinggiCanvasa - JB2 - (TinggiT_1)*c),
        Offset(JA + ((PanjangT_1 > PanjangT_2 ? PanjangT_1 : PanjangT_2) + LebarT)
            *c + JK - 5, TinggiCanvasa - JB2 - (TinggiT_1)*c),
        Line1);
    canvas.drawLine(
        Offset(JA + ((PanjangT_1 > PanjangT_2 ? PanjangT_1 : PanjangT_2) + LebarT)
            *c + JK, TinggiCanvasa - JB2 - (TinggiT_1)*c),
        Offset(JA + ((PanjangT_1 > PanjangT_2 ? PanjangT_1 : PanjangT_2) + LebarT)
            *c + JK, TinggiCanvasa - JB2 - (TinggiT_1 + TinggiT_2)*c),
        Line1);
    // vertikal
    //keterangan12  tinggi tangga
    canvas.rotate(270/180*pi);
    final Kete12 = Para(Text:TinggiT_1.toStringAsFixed(0) + " cm",Lebar:TinggiT_1*c,Posisi:TextAlign.center);
    canvas.drawParagraph(
        Kete12.toKetW(),
        Offset(-TinggiCanvasa + JB2 ,
            JA +
            (LebarT + (PanjangT_1 > PanjangT_2 ? PanjangT_1 : PanjangT_2))
                *c +JK));
    canvas.drawParagraph(
        Kete12.toKetW(),
        Offset( -TinggiCanvasa + JB,
            JA + (LebarT + (PanjangT_1 > PanjangT_2 ? PanjangT_1 : PanjangT_2+PanjangB_2))
                *c +JK));
    ...
    canvas.drawParagraph(
        Kete14.toKetW(),
        Offset(-TinggiCanvasa + JB2 + (TinggiT_1)*c,
            JA + (LebarT + (PanjangT_1 > PanjangT_2 ? PanjangT_1 : PanjangT_2))
                *c + JK));
    canvas.drawParagraph(
        Kete14.toKetW(),
...
class Gambar_Tangga_L_2 extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
    final double c = (1.0).toT_L_b();
    final double JA = 20;
    final double JB=50;
    //judulgambar
    ...
    path.lineTo((JA).toMirror(M, MV), TinggiCanvasb - JB - (LebarT + PanjangT_1)*c);
    path.lineTo((JA + (LebarT + PanjangT_2+PanjangB_2)*c).toMirror(M, MV),
        TinggiCanvasb - JB - (LebarT + PanjangT_1)*c);
    path.lineTo((JA + (LebarT + PanjangT_2+PanjangB_2)*c).toMirror(M, MV),
        TinggiCanvasb - JB - (PanjangT_1)*c);
    path.lineTo(
        (JA + (LebarT)*c).toMirror(M, MV), TinggiCanvasb - JB - (PanjangT_1)*c);
    path.lineTo((JA + (LebarT)*c).toMirror(M, MV), TinggiCanvasb - JB);
    canvas.drawPath(path, Line3);

    for (var i = 0; i <= OptredeNT_1-1; i++) {
      canvas.drawLine(
          Offset((JA).toMirror(M, MV), TinggiCanvasb - JB - (Antrede_1 * i)*c),
          Offset((JA + LebarT*c).toMirror(M, MV),
              TinggiCanvasb - JB - (Antrede_1 * i)*c),
          Line1);
    }
    for (var i = 0; i <= OptredeNT_2-1; i++) {
      canvas.drawLine(
          Offset((JA + (LebarT + Antrede_2 * i)*c).toMirror(M, MV),
              TinggiCanvasb - JB - PanjangT_1*c),
          Offset((JA + (LebarT + Antrede_2 * i)*c).toMirror(M, MV),
              TinggiCanvasb - JB + (-PanjangT_1 - LebarT)*c),
          Line1);
    }
    canvas.drawLine(
        Offset((JA).toMirror(M, MV), TinggiCanvasb - JB),
        Offset((JA).toMirror(M, MV), TinggiCanvasb - JB + (-PanjangT_1 - LebarT)*c),
        Line1);
    canvas.drawLine(
        Offset((JA).toMirror(M, MV), TinggiCanvasb - JB + (-PanjangT_1 - LebarT)*c),
        Offset((JA + (PanjangT_2 + LebarT)*c).toMirror(M, MV),
            TinggiCanvasb - JB + (-PanjangT_1 - LebarT)*c),
        Line1);
    canvas.drawLine(
        Offset((JA + LebarT*c).toMirror(M, MV), TinggiCanvasb - JB),
        Offset((JA + LebarT*c).toMirror(M, MV),
            TinggiCanvasb - JB + (-PanjangT_1)*c),
        Line1);
    canvas.drawLine(
    ...
    canvas.drawParagraph(Kete21.toKetW(),
        Offset((JA + (0.0)*c).toMirror(M, MV), TinggiCanvasb - JB + JK));

    //keterangan2 panjang 1
    final Kete22 = Para(Text: PanjangT_1.toStringAsFixed(0) + " cm",Lebar:PanjangT_1*c,Posisi: TextAlign.center);
    canvas.drawParagraph(
        Kete22.toKetW(),
        Offset((JA + (LebarT)*c).toMirror(M, MV),
            TinggiCanvasb - JB + JK));
    //keterangan2 panjang B atas
    final Kete25 = Para(Text: PanjangB_2.toStringAsFixed(0) + " cm",Lebar:PanjangB_2*c,Posisi: TextAlign.center);
    canvas.drawParagraph(
        Kete25.toKetW(),
        Offset((JA + (LebarT+PanjangT_2)*c).toMirror(M, MV),
            TinggiCanvasb - JB + JK));
    ...
    canvas.drawLine(Offset((JA).toMirror(M, MV), TinggiCanvasb - JB + JK),
        Offset((JA + (LebarT)*c).toMirror(M, MV), TinggiCanvasb - JB + JK), Line1);
    canvas.drawLine(
        Offset((JA + (LebarT)*c).toMirror(M, MV), TinggiCanvasb - JB + JK - 5),
        Offset((JA + (LebarT)*c).toMirror(M, MV), TinggiCanvasb - JB + JK + 5),
        Line1);
    canvas.drawLine(
        Offset((JA + (LebarT)*c).toMirror(M, MV), TinggiCanvasb - JB + JK),
        Offset((JA + (LebarT + PanjangT_2)*c).toMirror(M, MV), TinggiCanvasb - JB + JK),
        Line1);
    canvas.drawLine(
        Offset((JA + (LebarT+PanjangT_2)*c).toMirror(M, MV), TinggiCanvasb - JB + JK-5),
        Offset((JA + (LebarT + PanjangT_2)*c).toMirror(M, MV), TinggiCanvasb - JB + JK+5),
        Line1);
    canvas.drawLine(
        Offset((JA + (LebarT+PanjangT_2)*c).toMirror(M, MV), TinggiCanvasb - JB + JK),
        Offset((JA + (LebarT + PanjangT_2+PanjangB_2)*c).toMirror(M, MV), TinggiCanvasb - JB + JK),
        Line1);
    // kanan
    canvas.drawLine(
        Offset(JA + (LebarT + PanjangT_2+PanjangB_2)*c + JK, TinggiCanvasb - JB),
        Offset(JA + (LebarT + PanjangT_2+PanjangB_2)*c + JK,
            TinggiCanvasb - JB + (-PanjangT_1)*c),
        Line1);
    canvas.drawLine(
        Offset(JA + (LebarT + PanjangT_2+PanjangB_2)*c + JK - 5,
            TinggiCanvasb - JB + (-PanjangT_1)*c),
        Offset(JA + (LebarT + PanjangT_2+PanjangB_2)*c + JK + 5,
            TinggiCanvasb - JB + (-PanjangT_1)*c),
        Line1);
    ...
    canvas.drawParagraph(
        ...
        Offset(-TinggiCanvasb + JB  + (0.0)*c,
          JA + (LebarT + PanjangT_2+PanjangB_2)*c + JK
            ));
    //lebar
    canvas.drawParagraph(
        Kete21.toKetW(),
        Offset(-TinggiCanvasb + JB  + (PanjangT_1 )*c,
          JA + (LebarT + PanjangT_2+PanjangB_2)*c + JK
            ));
...
class Gambar_Tangga_L_3 extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
    //Judul31
...
class Gambar_Tangga_L_1m extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
    final double c = (1.0).toM_L_a();

    final double JB = 40;
    final double JB2 = JB;
    final double JA = 30+PanjangB_1*c;
    ...
    //keterangan
    final Kete = Para(Text:"D",font:font);
    canvas.drawParagraph(Kete.toKetW(),
        Offset((JA + (0-PanjangB_1)*c).toMirror(M, MV), TinggiCanvasa - JB + JK+(0.0)*c));
    final Kete1 = Para(Text:"E",font:font);
    canvas.drawParagraph(Kete1.toKetW(),
        Offset((JA + (0.0)*c).toMirror(M, MV), TinggiCanvasa - JB + JK+(0.0)*c));
    final Kete2 = Para(Text:"F",font:font);
    canvas.drawParagraph(Kete2.toKetW(),
        Offset((JA + (PanjangT_2)*c).toMirror(M, MV), TinggiCanvasa - JB + JK+(-TinggiT_2)*c));
    final Kete3 = Para(Text:"G",font:font);
    ...
    var path = Path();
    final double k=5;
    path.moveTo((JA-k + (-PanjangB_1)*c).toMirror(M, MV), TinggiCanvasa - JB-k + (0.0)*c );
    path.lineTo((JA+k + (-PanjangB_1)*c).toMirror(M, MV), TinggiCanvasa - JB-k + (0.0)*c );
    path.lineTo((JA+k + (-PanjangB_1)*c).toMirror(M, MV), TinggiCanvasa - JB+k + (0.0)*c );
    path.lineTo((JA-k + (-PanjangB_1)*c).toMirror(M, MV), TinggiCanvasa - JB+k + (0.0)*c );
    canvas.drawPath(path, Line3);

    path.moveTo((JA-k + (PanjangT_1+PanjangB_2)*c).toMirror(M, MV), TinggiCanvasa - JB-k + (-TinggiT_2)*c );
    path.lineTo((JA+k + (PanjangT_1+PanjangB_2)*c).toMirror(M, MV), TinggiCanvasa - JB-k + (-TinggiT_2)*c );
    path.lineTo((JA+k + (PanjangT_1+PanjangB_2)*c).toMirror(M, MV), TinggiCanvasa - JB+k + (-TinggiT_2)*c );
    path.lineTo((JA-k + (PanjangT_1+PanjangB_2)*c).toMirror(M, MV), TinggiCanvasa - JB+k + (-TinggiT_2)*c );
    canvas.drawPath(path, Line3);
  }
...
class Gambar_Tangga_L_2m extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
    final double c = (1.0).toM_L_b();
...
class Gambar_Tangga_L_1O extends CustomPainter {
  final c = (1.0).toT_L_a();
...
//tangga atas
class Gambar_Tangga_L_2O extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
...
class Gambar_Tangga_L_3O extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size)  {
    final double c = (1.0).toG_L_c();
   ...
   final double gaya =20;
    ...
    //GHIJ
    //keterangan MaGH
    final Kete2 = Para(Text:MaGH.toStringAsFixed(1) + " kNm",);
    canvas.drawParagraph(Kete2.toKetW(),
        Offset((JA-20+(-PanjangB_1*7/10)*c).toMirror(M, MV),TinggiCanvasa-JB+20+20+(0.0)*c));
//keterangan beban FHGH
    final Kete5 = Para(Text:FHbGH.toStringAsFixed(1) + " kN",);
    canvas.drawParagraph(Kete5.toKetW(),
        Offset((JA-JK*5+(-PanjangB_1)*c).toMirror(M, MV),
            TinggiCanvasa-JB*1-JK*4+(0.0)*c));
//keterangan beban FVGH
    final Kete6 = Para(Text:FVbGH.toStringAsFixed(1) + " kN",);
...
//keterangan beban terfaktor
    final Kete20 = Para(Text:BebanTerfaktorBordes_1.toStringAsFixed(1) + " kN/m",Lebar:PanjangB_1*c,Posisi: TextAlign.center);
...
//keterangan beban terfaktor
    final Kete4 = Para(Text:BebanTerfaktorTangga_2.toStringAsFixed(1) + " kN/m",Lebar: PanjangT_2*c,Posisi: TextAlign.center);
    ...
    canvas.drawLine(
        ...
        Offset((JA+(PanjangT_1+PanjangB_2)*c).toMirror(M, MV), TinggiCanvasa - JB2-gaya+(-TinggiT_1)*c),
        Line9);
    for (var i = 0; i <= (PanjangB_2/20).toRound(0); i++) {
      canvas.drawLine(
          Offset((JA+(PanjangT_1+PanjangB_2/(PanjangB_2/20).toRound(0)*i)*c).toMirror(M, MV),
              TinggiCanvasa - JB2+(-TinggiT_1)*c),
          Offset((JA+(PanjangT_1+PanjangB_2/(PanjangB_2/20).toRound(0)*i)*c).toMirror(M, MV),
      ...
      canvas.drawLine(
          Offset((JA+(PanjangT_1+PanjangB_2/(PanjangB_2/20).toRound(0)*i)*c).toMirror(M, MV),
              TinggiCanvasa - JB2+(-TinggiT_1)*c),
          Offset((JA+5+(PanjangT_1+PanjangB_2/(PanjangB_2/20).toRound(0)*i)*c).toMirror(M, MV),
    ...
    //kanan
    arc1.moveTo((JA+(PanjangT_1+PanjangB_2*5/10)*c).toMirror(M, MV),
        TinggiCanvasa-JB2+20+(-TinggiT_1)*c);
    arc1.arcToPoint(
        Offset((JA+(PanjangT_1+PanjangB_2*5/10)*c).toMirror(M, MV),
    ...
    canvas.drawLine(
        Offset((JA+(PanjangT_1+PanjangB_2*5/10)*c).toMirror(M, MV),
            TinggiCanvasa-JB2+40+(-TinggiT_1)*c),
        Offset((JA+4.5+(PanjangT_1+PanjangB_2*5/10)*c).toMirror(M, MV),
    ...
    //JIH
    canvas.drawLine(
        Offset((JA+JK+(PanjangT_1+PanjangB_2)*c).toMirror(M, MV),
            TinggiCanvasa-JB2+(-TinggiT_1)*c),
        Offset((JA+JK+20+(PanjangT_1+PanjangB_2)*c).toMirror(M, MV),
            TinggiCanvasa-JB2+(-TinggiT_1)*c),
        Line9);
    canvas.drawLine(
        Offset((JA+JK+(PanjangT_1+PanjangB_2)*c).toMirror(M, MV),
            TinggiCanvasa-JB2+(-TinggiT_1)*c),
        Offset((JA+JK+5+(PanjangT_1+PanjangB_2)*c).toMirror(M, MV),
            TinggiCanvasa-JB2+5+(-TinggiT_1)*c),
        Line9);
    canvas.drawLine(
        Offset((JA+JK+(PanjangT_1+PanjangB_2)*c).toMirror(M, MV),
    ...
    //V
    canvas.drawLine(
        Offset((JA+(PanjangT_1+PanjangB_2)*c).toMirror(M, MV),
    ...
    canvas.drawLine(
        Offset((JA+(PanjangT_1+PanjangB_2)*c).toMirror(M, MV),
            TinggiCanvasa-JB2+JK+(-TinggiT_1)*c),
        Offset((JA-5+(PanjangT_1+PanjangB_2)*c).toMirror(M, MV),
...
//IJ
//keterangan MaJI
    final Kete12 = Para(Text:(-MaJI).toStringAsFixed(1) + " kNm");
    canvas.drawParagraph(Kete12.toKetW(),
        Offset((JA-20+(PanjangT_1+PanjangB_2*4/5)*c).toMirror(M, MV),TinggiCanvasa-JB2+60+(-TinggiT_1)*c));
//keterangan beban terfaktor
    final Kete13 = Para(Text:BebanTerfaktorBordes_2.toStringAsFixed(1) + " kN/m",Lebar:PanjangB_2*c,Posisi: TextAlign.center);
    canvas.drawParagraph(Kete13.toKetW(),
        Offset((JA+(PanjangT_1)*c).toMirror(M, MV),TinggiCanvasa-JB2-gaya-30+(-TinggiT_1)*c));
//keterangan beban FHIJ
    final Kete16 = Para(Text:(-FHbJI).toStringAsFixed(1) + " kN",);
    canvas.drawParagraph(Kete16.toKetW(),
        Offset((JA+JK+(PanjangT_1+PanjangB_2)*c).toMirror(M, MV),
            TinggiCanvasa-JB2*1-JK*4+(-TinggiT_1)*c));
//keterangan beban FVIJ
    final Kete17 = Para(Text:FVbJI.toStringAsFixed(1) + " kN",);
    canvas.drawParagraph(Kete17.toKetW(),
        Offset((JA+JK+(PanjangT_1+PanjangB_2)*c).toMirror(M, MV)
            ,TinggiCanvasa-JB2+JK*3+(-TinggiT_1)*c));
  }
...
class Gambar_Tangga_L_5O extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
    final double c = (1.0).toG_L_e();
    final double s = 1;
...
// keterangan NHI
    ...
    canvas.drawParagraph(Kete4.toKetW(),
        Offset((JA-JK*3+(-NHI*s*cs)*c).toMirror(M, MV),
...
// keterangan NIH
    ...
    canvas.drawParagraph(Kete1.toKetW(),
        Offset((JA-JK*4+(PanjangT_2)*c).toMirror(M, MV),
            TinggiCanvasa -JB-JK+(-TinggiT_2-NIH*s*cc)*c));
...
// keterangan NJI
    ...
    canvas.drawParagraph(Kete6.toKetW(),
        Offset((JA-JK*2+(PanjangT_2+PanjangB_2)*c).toMirror(M, MV),
            TinggiCanvasa - JB+(-TinggiT_2+NJI*s)*c));
...
class Gambar_Tangga_L_6O extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
    final double c =(1.0).toG_L_f();
    final s = 2;
    final double font =16;
    final double JB = 50;
...
// keterangan SIH
    ...
    canvas.drawParagraph(Kete5.toKetW(),
        Offset((JA-JK*7+(PanjangT_2)*c).toMirror(M, MV),
...
class Gambar_Tangga_L_7O extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
    final double c = (1.0).toG_L_g();
    final double s = 3;
    final double font = 16;
    final double JB = 40;
    final double JA = 50+PanjangB_1*c;
...
// keterangan MBA
    ...
    canvas.drawParagraph(Kete1.toKetW(),
        Offset((JA-JK*7+(PanjangT_2+MIH*4*cs)*c).toMirror(M, MV),
            TinggiCanvasa - JB-JK*1+(-TinggiT_2)*c));
// keterangan MLAB
...
//tangga bawah
class Gambar_Tangga_L_8O extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
    final double c = (1.0).toG_L_h();
...
class Gambar_Tangga_L_9O extends CustomPainter {

  @override
  void paint(Canvas canvas, Size size) {
    final double c = (1.0).toG_L_i();
...
//garis gaya //AB //H
    ...
    canvas.drawParagraph(Kete2.toKetW(),
        Offset((JA+(PanjangT_1*5/10)*c).toMirror(M, MV),TinggiCanvasa-JB+20+20+(-TinggiT_1*5/10)*c));
//keterangan MaBA
...
class Gambar_Tangga_L_11O extends CustomPainter {

  @override
  void paint(Canvas canvas, Size size) {
    final double c = (1.0).toG_L_k();
    final double s = 1;
    final double font = 16;
    final double JB = 30;
    final double JA = 70;
...
// keterangan NBA
    ...
    canvas.drawParagraph(Kete1.toKetW(),
        Offset((JA-JK*6+(PanjangT_1)*c).toMirror(M, MV),
...
class Gambar_Tangga_L_12O extends CustomPainter {

  @override
  void paint(Canvas canvas, Size size) {
    final double c =(1.0).toG_L_l();
    final s = 4;
    final double font =16;
    final double JB = 40;
    final double JA = 60;
...
class Gambar_Tangga_L_13O extends CustomPainter {

  @override
  void paint(Canvas canvas, Size size) {
    final double c = (1.0).toG_L_m();
    final double s = 6;
...
//diameter ..atas
class Gambar_Tangga_L_1G extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
    final double c = (1.0).toD_L_a();
    ...
    final double Antrede_2=AntredeR_2;
    final double Optrede_2=OptredeR_2;
    final double xT_2=TebalPelatTanggaPilih_2/sina_2;
    final double yT_2=TebalPelatTanggaPilih_2/sina_2+(Optrede_2-TebalPelatBordesPilih_2)*cosa_2*sina_2;
    final double yB_2=TebalPelatBordesPilih_2;
    final double STx = (Selimut/10)/sina_2;//jarak X tulangan tangga bawah
    final double SATx = Antrede_2+(Selimut/10)/sina_2-(Selimut/10)/sina_2*cosa_2;//jarak X tulangan tangga atas kiri
    final double SBTx=-(Selimut/10)/sina_2+(Selimut/10)/sina_2*cosa_2;//jarak X tulangan tangga atas kanan
    final double SBy=(Selimut/10);//jarak tulangan bordes
    final double SDB=2;//jarak bulat
    final double SDBx=2*sina_2;//jarak bulat x
    final double SDBy=2*cosa_2;//jarak bulat y
    ...
    path.lineTo((JA + (Antrede_2 * (AntredeNT_2)+PanjangB_2)*c).toMirror(M, MV),
        TinggiCanvasa - JB - Optrede_2*c * (OptredeNT_2));
    path.lineTo((JA + (Antrede_2 * (AntredeNT_2)+PanjangB_2)*c).toMirror(M, MV),
        TinggiCanvasa - JB + ((-Optrede_2*(OptredeNT_2))+yB_2)*c);
    path.lineTo((JA + (Antrede_1* (AntredeNT_1-_teratas)+yT_2)*c).toMirror(M, MV),
        TinggiCanvasa - JB + ((-Optrede_2*OptredeNT_2)+yB_2)*c);
    path.lineTo((JA + (-Antrede_2+yT_2)*c*2).toMirror(M, MV),
        TinggiCanvasa - JB+(yB_2)*c);
...
//..bawah
    ...
    for (var i = 0; i <= OptredeNT_2-1; i++) {
      canvas.drawLine(
          Offset((JA+(Antrede_2*i)*c).toMirror(M, MV), TinggiCanvasa - JB+(-Optrede_2*(i))*c),
          Offset((JA+(Antrede_2*i)*c).toMirror(M, MV), TinggiCanvasa - JB+(-Optrede_2*(i+1))*c),
          Line1);
      canvas.drawLine(
          Offset((JA+(Antrede_2*(i))*c).toMirror(M, MV), TinggiCanvasa - JB+(-Optrede_2*(i+1))*c),
    ...
    canvas.drawLine(
        Offset((JA+(PanjangT_2+PanjangB_2-bB/2)*c).toMirror(M, MV),
            TinggiCanvasa - JB+(-TinggiT_2+yB_2)*c),
        Offset((JA+(PanjangT_2-Antrede_2*_teratas+yT_2)*c).toMirror(M, MV),
            TinggiCanvasa - JB+(-TinggiT_2+yB_2)*c),
        Line1);
    canvas.drawLine(
        Offset((JA+(PanjangT_2-Antrede_2*_teratas+yT_2)*c).toMirror(M, MV),
            TinggiCanvasa - JB+(-TinggiT_2+yB_2)*c),
        Offset((JA+(-Antrede_2+yT_2)*2*c).toMirror(M, MV),
            TinggiCanvasa - JB+(yB_2)*c),
        Line1);
    canvas.drawLine(
        Offset((JA+(-Antrede_2+yT_2)*2*c).toMirror(M, MV),
...
//garis bawah
    ...
    canvas.drawLine(
        ...
        Offset(JA + (PanjangT_2+SATx+(-Antrede_2*_teratas))*c,
            TinggiCanvasa - JB +(-TinggiT_2+SBy)*c),
        Line11);
    canvas.drawLine(
        Offset((JA+(xT_2-STx)*c).toMirror(M, MV),
            TinggiCanvasa - JB+(0.0)*c),
        Offset((JA+(PanjangT_2-Antrede_2*_teratas+yT_2+SBTx)*c).toMirror(M, MV),
            TinggiCanvasa - JB+(-TinggiT_2+yB_2-SBy)*c),
        Line11);
    canvas.drawLine(
        Offset((JA+(PanjangT_2-Antrede_2*_teratas+yT_2+SBTx)*c).toMirror(M, MV),
            TinggiCanvasa - JB+(-TinggiT_2+yB_2-SBy)*c),
        Offset((JA+(PanjangT_2-Antrede_2*_teratas+yT_2+SBTx+yB_2/2/sina_2*cosa_2)*c).toMirror(M, MV),
            TinggiCanvasa - JB+(-TinggiT_2+yB_2-SBy-yB_2/2)*c),
        Line11);
    canvas.drawLine(
        Offset((JA+(PanjangT_2-Antrede_2*_teratas+yT_2+SBTx+yB_2/2/sina_2*cosa_2)*c).toMirror(M, MV),
            TinggiCanvasa - JB+(-TinggiT_2+yB_2-SBy-yB_2/2)*c),
        Offset((JA+(PanjangT_2-Antrede_2*_teratas+yT_2+SBTx+yB_2/2/sina_2*cosa_2+4*SDBx*sina_2)*c).toMirror(M, MV),
            TinggiCanvasa - JB+(-TinggiT_2+yB_2-SBy-yB_2/2+4*SDBx*cosa_2)*c),
        Line11);
    canvas.drawLine(
        Offset((JA+(PanjangT_2-Antrede_2*_teratas+yT_2+SBTx+yB_2/2/sina_2*cosa_2+4*SDBx*sina_2)*c).toMirror(M, MV),
            TinggiCanvasa - JB+(-TinggiT_2+yB_2-SBy-yB_2/2+4*SDBx*cosa_2)*c),
        Offset((JA+(PanjangT_2-Antrede_2*_teratas+yT_2+SBTx+yB_2/4/sina_2*cosa_2+4*SDBx*sina_2)*c).toMirror(M, MV),
            TinggiCanvasa - JB+(-TinggiT_2+yB_2-SBy-yB_2/4+4*SDBx*cosa_2)*c),
        Line11);


    for (var i = 0; i <= (PanjangTanggaMiring_2*100/SGT_2); i++) {
      canvas.drawCircle(
          Offset(JA + (STx+SDBx+(PanjangT_2-STx+SATx+(-Antrede_2*_teratas))/(PanjangTanggaMiring_2*100/SGT_2)*i) * c,
              TinggiCanvasa - JB+(SDBy+(-TinggiT_2+SBy)/(PanjangTanggaMiring_2*100/SGT_2)*i)*c),
          1.2, Line11);
    }
    for (var i = 0; i <= PanjangTanggaMiring_2*100/SGT_2; i++) {
      canvas.drawCircle(
          Offset(JA + (xT_2-STx-SDBx+(PanjangT_2-Antrede_2*_teratas+yT_2+SBTx-(xT_2-STx))/(PanjangTanggaMiring_2*100/SGT_2)*i) * c,
              TinggiCanvasa - JB+(-SDBy+(-TinggiT_2+yB_2-SBy)/(PanjangTanggaMiring_2*100/SGT_2)*i)*c),
          1.2, Line11);
    }
    ...
    canvas.drawLine(
        ...
        Offset(JA-JK*7 + (PanjangT_2+PanjangB_2*4/20-Selimut/10)*c,
            TinggiCanvasa - JB -JK*4 +(-TinggiT_2+Selimut/10)*c),
        Line11);
    final KeteD5 = Para(Text:"D$Dtul_12-$Stul_12");
    canvas.drawParagraph(
        KeteD5.toKetW(),
        Offset(JA-JK*7 + (PanjangT_2+PanjangB_2*4/20-Selimut/10)*c,
...
//TGF
    ...
    canvas.drawLine(
        ...
        Offset(JA-JK*7 + (PanjangT_2+PanjangB_2*16/20-Selimut/10)*c,
            TinggiCanvasa - JB+JK*4 + (-TinggiT_2-Selimut/10+Optrede_2)*c),
        Line11);
    final KeteD6 = Para(Text:"D$Dtul_11-$Stul_11");
    canvas.drawParagraph(
        KeteD6.toKetW(),
        Offset(JA-JK*7 + (PanjangT_2+PanjangB_2*16/20-Selimut/10)*c,
...
//GFG
    ...
    canvas.drawLine(
        ...
        Line11);
    final KeteD7 = Para(Text:"D$Dtul_13-$Stul_13");
    canvas.drawParagraph(
        KeteD7.toKetW(),
        Offset(JA+JK*2 + (PanjangT_2+PanjangB_2/(PanjangB_2/SGBb_2)*(PanjangB_2/SGBb_2/2).round()) * c,
    ...
    canvas.drawLine(
        ...
        Offset(JA+JK*8 + (-PanjangB_1*16/20-Selimut/10)*c,
            TinggiCanvasa - JB +JK*6 +(-Selimut/10+Optrede_2)*c),
        Line11);
    final KeteD52 = Para(Text:"D$Dtul_5-$Stul_5");
    canvas.drawParagraph(
        KeteD52.toKetW(),
        Offset(JA+JK*3 + (-PanjangB_1*16/20-Selimut/10)*c,
...
//TED
    ...
    canvas.drawLine(
        ...
        Offset(JA-JK*5 + (-PanjangB_1*4/20-Selimut/10)*c,
            TinggiCanvasa - JB-JK*2 + (Selimut/10)*c),
        Line11);
    final KeteD62 = Para(Text:"D$Dtul_4-$Stul_4");
    canvas.drawParagraph(
        KeteD62.toKetW(),
        Offset(JA-JK*5 + (-PanjangB_1*4/20+Selimut/10)*c,
...
//balok ...bawah
class Gambar_Tangga_L_2G extends CustomPainter {

  @override
  void paint(Canvas canvas, Size size) {
    final double c = (1.0).toD_L_b();
    final double JA = 70;
...
//geser
    canvas.drawLine(
        ...
        Offset((JA2-JK*1+(selimutB/10)*c).toMirror(M, MV),
            TinggiCanvasa - JB+(-hB*8/10+selimutB/10)*c),
        Line11);
    final KeteD32 = Para(Text:"D$Dtul_3-${JBGmina_2.toText()}");
    canvas.drawParagraph(
        KeteD32.toKetW(),
...
//...atas
class Gambar_Tangga_L_3G extends CustomPainter {

  @override
  void paint(Canvas canvas, Size size) {
    final double c = (1.0).toD_L_c();
    final double JA = 70;
    final double JA2 = LebarCanvas/2+70;
    final double JB = 70;
...
//garis selimut
    ...
    canvas.drawLine(
        ...
        Offset((JA-JK*1+(bB-selimutB/10*1.2)*c).toMirror(M, MV),
            TinggiCanvasa -JK*2- JB+(-hB+selimutB/10*1.2)*c),
        Line11);
    final KeteD1 = Para(Text:"$Stul_14 D-$Dtul_14");
    canvas.drawParagraph(
        KeteD1.toKetW(),
        Offset((JA-JK*7+(selimutB/10*1.2)*c).toMirror(M, MV),
            TinggiCanvasa -JK*3-JB+(-hB+selimutB/10*1.2)*c));
    ...
    canvas.drawLine(
        ...
        Offset((JA-JK*1+(bB-selimutB/10*1.2)*c).toMirror(M, MV),
            TinggiCanvasa -JK*2- JB+(-selimutB/10*1.2)*c),
        Line11);
    final KeteD2 = Para(Text:"$Stul_15 D-$Dtul_15");
    canvas.drawParagraph(
        KeteD2.toKetW(),
        Offset((JA-JK*7+(selimutB/10*1.2)*c).toMirror(M, MV),
            TinggiCanvasa -JK*3- JB+(-selimutB/10*1.2)*c));
//geser
    canvas.drawLine(
        ...
        Offset((JA-JK*1+(selimutB/10)*c).toMirror(M, MV),
            TinggiCanvasa - JB+(-hB*8/10+selimutB/10)*c),
        Line11);
    final KeteD3 = Para(Text:"D$Dtul_16-$Stul_16");
    canvas.drawParagraph(
        KeteD3.toKetW(),
        Offset((JA-JK*7+(selimutB/10)*c).toMirror(M, MV),
            TinggiCanvasa - JB-JK+(-hB*8/10+selimutB/10)*c));
...
//garis selimut
    ...
    canvas.drawLine(
        ...
        Offset((JA2-JK*1+(bB-selimutB/10*1.2)*c).toMirror(M, MV),
            TinggiCanvasa -JK*2- JB+(-hB+selimutB/10*1.2)*c),
        Line11);
    final KeteD12 = Para(Text:"$Stul_14 D-$Dtul_14");
    canvas.drawParagraph(
        KeteD12.toKetW(),
        Offset((JA2-JK*7+(selimutB/10*1.2)*c).toMirror(M, MV),
            TinggiCanvasa -JK*3-JB+(-hB+selimutB/10*1.2)*c));
    ...
    canvas.drawLine(
        ...
        Offset((JA2-JK*1+(bB-selimutB/10*1.2)*c).toMirror(M, MV),
            TinggiCanvasa -JK*2- JB+(-selimutB/10*1.2)*c),
        Line11);
    final KeteD22 = Para(Text:"$Stul_15 D-$Dtul_15");
    canvas.drawParagraph(
        KeteD22.toKetW(),
        Offset((JA2-JK*7+(selimutB/10*1.2)*c).toMirror(M, MV),
            TinggiCanvasa -JK*3- JB+(-selimutB/10*1.2)*c));
//geser
    canvas.drawLine(
        ...
        Offset((JA2-JK*1+(selimutB/10)*c).toMirror(M, MV),
            TinggiCanvasa - JB+(-hB*8/10+selimutB/10)*c),
        Line11);
    final KeteD32 = Para(Text:"D$Dtul_16-${JBGminb_2.toText()}");
    canvas.drawParagraph(
        KeteD32.toKetW(),
        Offset((JA2-JK*7+(selimutB/10)*c).toMirror(M, MV),
            TinggiCanvasa - JB-JK+(-hB*8/10+selimutB/10)*c));
...
//..bawah
class Gambar_Tangga_L_4G extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
    final double c = (1.0).toD_L_d();
    final double JA = 50;
    final double JB = 40;
...
//balok
class Gambar_Tangga_L_5G extends CustomPainter {
...
//geser
    canvas.drawLine(
        ...
        Offset((JA2-JK*1+(selimutB/10)*c).toMirror(M, MV),
            TinggiCanvasa - JB+(-hB*8/10+selimutB/10)*c),
        Line11);
    final KeteD32 = Para(Text:"D$Dtul_26-${JBGmina_1.toText()}");
    canvas.drawParagraph(
        KeteD32.toKetW(),
        Offset((JA2-JK*7+(selimutB/10)*c).toMirror(M, MV),
            TinggiCanvasa - JB-JK+(-hB*8/10+selimutB/10)*c));
...
Future PdfGambar1_L() async {
  ...
  canvas.drawParagraph(KeteALP_A.toKetB(),
      Offset((JA-JK+(PanjangT_1+LebarB_1)*c).toMirror(M, MV) ,
          TinggiCanvasa - JB2-JK +(-Optrede_1*2)*c));
  final KeteALP_B = Para(Text:"B");
  canvas.drawParagraph(KeteALP_B.toKetB(),
      Offset(
          (JA-JK+(LebarB_1)*c).toMirror(M, MV) ,
          TinggiCanvasa - JB2+JK+(-TinggiT_1)*c));
  ...
  canvas.drawParagraph(KeteALP_D.toKetB(),
      Offset(
          (JA+(0.0)*c).toMirror(M, MV) ,
          TinggiCanvasa - JB-JK*1.5+(-TinggiT_1)*c));
  ...
  canvas.drawParagraph(KeteALP_F.toKetB(),
      Offset((JA-JK+(PanjangT_2+LebarB_1)*c).toMirror(M, MV) ,
          TinggiCanvasa - JB-JK*1.5+(-TinggiT_1-TinggiT_2)*c));
  final KeteALP_G = Para(Text:"G");
  canvas.drawParagraph(KeteALP_G.toKetB(),
      Offset((JA-JK+(PanjangT_2+LebarB_1+PanjangB_2)*c).toMirror(M, MV) ,
          TinggiCanvasa - JB-JK*1.5+(-TinggiT_1-TinggiT_2)*c));
    //gambar tangga atas
    var path1 = Path();
    path1.moveTo((JA).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_1 - TinggiT_2)*c);
    path1.lineTo((JA + (LebarT)*c).toMirror(M, MV),
        TinggiCanvasa - JB2 + (-TinggiT_1 - TinggiT_2)*c);
    path1.lineTo(
        (JA + (LebarT)*c).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_2)*c);
    for (var i = 0; i <= OptredeNT_1 - 1; i++) {
      path1.lineTo((JA + (LebarT + (i + 1) * Antrede_1)*c).toMirror(M, MV),
          TinggiCanvasa - JB2 + (-TinggiT_2 + i * Optrede_1)*c);
      path1.lineTo((JA + (LebarT + (i + 1) * Antrede_1)*c).toMirror(M, MV),
          TinggiCanvasa - JB2 + (-TinggiT_2 + (i + 1) * Optrede_1)*c);
    }
    path1.lineTo(
        (JA + (LebarT + PanjangT_1 - Antrede_1)*c).toMirror(M, MV),
        TinggiCanvasa - JB2);
    path1.lineTo((JA + (LebarT)*c).toMirror(M, MV),
        TinggiCanvasa - JB2 + (-TinggiT_1 + Optrede_1)*c);
    path1.lineTo((JA).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_1 + Optrede_1)*c);
    canvas.drawPath(path1, Line3);

    canvas.drawLine(
        Offset((JA).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_1 - TinggiT_2)*c),
        Offset((JA + LebarT*c).toMirror(M, MV),
            TinggiCanvasa - JB2 + (-TinggiT_1 - TinggiT_2)*c),
        Line1);
    canvas.drawLine(
    ...
    for (var i = 0; i <= OptredeNT_1 - 1; i++) {
      canvas.drawLine(
          Offset((JA + (LebarT + Antrede_1 * i)*c).toMirror(M, MV),
              TinggiCanvasa - JB2 + (-TinggiT_1 + Optrede_1 * i)*c),
          Offset((JA + (LebarT + Antrede_1 * (i + 1))*c).toMirror(M, MV),
              TinggiCanvasa - JB2 + (-TinggiT_1 + Optrede_1 * i)*c),
          Line1);
      canvas.drawLine(
          Offset((JA + (LebarT + Antrede_1 * (i + 1))*c).toMirror(M, MV),
              TinggiCanvasa - JB2 + (-TinggiT_1 + Optrede_1 * i)*c),
          Offset(
              (JA + (LebarT + Antrede_1 * (i + 1))*c).toMirror(M, MV),
              TinggiCanvasa - JB2 + (-TinggiT_1 + Optrede_1 * (i + 1))*c),
          Line1);
    }
    canvas.drawLine(
        Offset((JA + (LebarT + PanjangT_1)*c).toMirror(M, MV), TinggiCanvasa - JB2),
        Offset((JA + (LebarT + PanjangT_1 - Antrede_1)*c).toMirror(M, MV),
            TinggiCanvasa - JB2),
        Line1);
    canvas.drawLine(
        Offset((JA + (LebarT + PanjangT_1 - Antrede_1)*c).toMirror(M, MV),
    ...
    canvas.drawLine(
        Offset((JA + (LebarT)*c).toMirror(M, MV),
            TinggiCanvasa - JB2 + (-TinggiT_2 + Optrede_1)*c),
        Offset((JA).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_1 + Optrede_1)*c),
        Line1);
    canvas.drawLine(
        Offset((JA).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_1 + Optrede_1)*c),
        Offset((JA).toMirror(M, MV), TinggiCanvasa - JB2 + (-TinggiT_1 - TinggiT_2)*c),
    ...
    for (var i = 0; i <= OptredeNT_1 - 1; i++) {
      canvas.drawLine(
          Offset((JA).toMirror(M, MV),
              TinggiCanvasa - JB2 + (-TinggiT_1 - TinggiT_2 + Optrede_2 * (i + 1))*c),
          Offset(
              (JA + (LebarT)*c).toMirror(M, MV),
              TinggiCanvasa - JB2 +
                  (-TinggiT_1 - TinggiT_2 + Optrede_2 * (i + 1))*c),
          Line1);
    }
    ...
    canvas.drawParagraph(
        Kete15.toKetB(),
        Offset(JA + (LebarT + PanjangT_1 * 2 / 5)*c,
            TinggiCanvasa - JB2-JK - Optrede_1*c));
