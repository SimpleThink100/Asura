...
void generatePdfGambar1_I_1B_1() async {
  final int r=4;
  final int x=340*r;
  final int y=220*r;
  ...
  //keterangan11 panjang tangga
  final Kete = Para(Text:PanjangT_1.toStringAsFixed(0) + " cm",Lebar: PanjangT_1.toT_I_1B_a(),Posisi: TextAlign.center);
  canvas.drawParagraph(Kete.toKetB(),
      Offset((JA).toMirror(M, MV) , TinggiCanvasa - JB + JK));

  //keterangan14 panjang bordes
  final Kete14 = Para(Text: PanjangB_1.toStringAsFixed(0) + " cm",Lebar: PanjangB_1.toT_I_1B_a(),Posisi: TextAlign.center);
  canvas.drawParagraph(Kete14.toKetB(),
      Offset((JA + PanjangT_1.toT_I_1B_a()).toMirror(M, MV), TinggiCanvasa - JB + JK));

  //keterangan13
  final Kete13 = Para(Text: Sudut_1.toStringAsFixed(0) + " °");
  canvas.drawParagraph(
      Kete13.toKetB(),
      Offset(JA + (PanjangT_1 / 3).toT_I_1B_a(),
          TinggiCanvasa - JB - (TinggiT_1 / 5).toT_I_1B_a()));

  //Gambar12
  for (var i = 0; i <= OptredeNT_1-1; i++) {
    canvas.drawLine(
        Offset((JA+(Antrede_1*i).toT_I_1B_a()).toMirror(M, MV), TinggiCanvasa - JB+(-Optrede_1*(i)).toT_I_1B_a()),
        Offset((JA+(Antrede_1*i).toT_I_1B_a()).toMirror(M, MV), TinggiCanvasa - JB+(-Optrede_1*(i+1)).toT_I_1B_a()),
        Line5);
    canvas.drawLine(
        Offset((JA+(Antrede_1*(i)).toT_I_1B_a()).toMirror(M, MV), TinggiCanvasa - JB+(-Optrede_1*(i+1)).toT_I_1B_a()),
        Offset((JA+(Antrede_1*(i+1)).toT_I_1B_a()).toMirror(M, MV), TinggiCanvasa - JB+(-Optrede_1*(i+1)).toT_I_1B_a()),
        Line5);
  }
  canvas.drawLine(
      Offset((JA+(PanjangT_1).toT_I_1B_a()).toMirror(M, MV), TinggiCanvasa - JB+(-TinggiT_1).toT_I_1B_a()),
      Offset((JA+(PanjangT_1+PanjangB_1).toT_I_1B_a()).toMirror(M, MV), TinggiCanvasa - JB+(-TinggiT_1).toT_I_1B_a()),
      Line5);
  canvas.drawLine(
      Offset((JA+(PanjangT_1+PanjangB_1).toT_I_1B_a()).toMirror(M, MV), TinggiCanvasa - JB+(-TinggiT_1).toT_I_1B_a()),
      Offset((JA+(PanjangT_1+PanjangB_1).toT_I_1B_a()).toMirror(M, MV), TinggiCanvasa - JB+(-TinggiT_1+Optrede_1).toT_I_1B_a()),
      Line5);
  canvas.drawLine(
      Offset((JA+(PanjangT_1+PanjangB_1).toT_I_1B_a()).toMirror(M, MV), TinggiCanvasa - JB+(-TinggiT_1+Optrede_1).toT_I_1B_a()),
      Offset((JA+(PanjangT_1).toT_I_1B_a()).toMirror(M, MV), TinggiCanvasa - JB+(-TinggiT_1+Optrede_1).toT_I_1B_a()),
      Line5);
  canvas.drawLine(
      Offset((JA+(PanjangT_1).toT_I_1B_a()).toMirror(M, MV), TinggiCanvasa - JB+(-TinggiT_1+Optrede_1).toT_I_1B_a()),
      Offset((JA+(Antrede_1).toT_I_1B_a()).toMirror(M, MV), TinggiCanvasa - JB+(0.0).toT_I_1B_a()),
      Line5);
  canvas.drawLine(
      Offset((JA+(Antrede_1).toT_I_1B_a()).toMirror(M, MV), TinggiCanvasa - JB+(0.0).toT_I_1B_a()),
      Offset((JA+(0.0).toT_I_1B_a()).toMirror(M, MV), TinggiCanvasa - JB+(0.0).toT_I_1B_a()),
      Line5);
//garis bawah
  canvas.drawLine(
      Offset((JA).toMirror(M, MV), TinggiCanvasa - JB + JK),
      ...
      Line5);
  canvas.drawLine(
      Offset((JA + PanjangT_1.toT_I_1B_a()).toMirror(M, MV), TinggiCanvasa - JB + JK-5),
      Offset((JA + (PanjangT_1).toT_I_1B_a()).toMirror(M, MV), TinggiCanvasa - JB + JK+5),
      Line5);
  canvas.drawLine(
      Offset((JA + PanjangT_1.toT_I_1B_a()).toMirror(M, MV), TinggiCanvasa - JB + JK),
      Offset((JA + (PanjangT_1+PanjangB_1).toT_I_1B_a()).toMirror(M, MV), TinggiCanvasa - JB + JK),
      Line5);
  //garis kanan
  canvas.drawLine(
      Offset((JA + (PanjangT_1+PanjangB_1).toT_I_1B_a() + JK).toMirror(M, MV), TinggiCanvasa - JB),
      Offset((JA + (PanjangT_1+PanjangB_1).toT_I_1B_a() + JK).toMirror(M, MV),
          TinggiCanvasa - JB - TinggiT_1.toT_I_1B_a()),
      Line5);

  // keterangan vertikal
  //keterangan12 tinggi
  canvas.rotate(270/180*pi);
  final Kete12 = Para(Text: TinggiT_1.toStringAsFixed(0) + " cm",Lebar: TinggiT_1.toT_I_1B_a(),Posisi: TextAlign.center);
  canvas.drawParagraph(
      Kete12.toKetB(),
      Offset(-TinggiCanvasa+JB,
          JA+(PanjangT_1+PanjangB_1).toT_I_1B_a()+JK));
...
void generatePdfGambar2_I_1B_2() async {
  final int r =4;
  final int x=170*r;
  final int y=80*r;
  final recorder = ui.PictureRecorder();
  ...
  canvas.drawLine(
      Offset((JA + (0.0).toG_I_b() + JK).toMirror(M, MV),
          (TinggiCanvasa - JB+(0.00).toG_I_b())),
      Offset((JA + (0.0).toG_I_b() + JK).toMirror(M, MV),
          (TinggiCanvasa - JB +(-Optrede_1).toG_I_b())),
      Line5);
  canvas.drawLine(
      Offset(((JA + (0.0).toG_I_b() + JK).toMirror(M, MV)),
          (TinggiCanvasa - JB+(-Optrede_1).toG_I_b())),
      Offset((JA + (Antrede_1).toG_I_b() + JK).toMirror(M, MV),
          (TinggiCanvasa - JB +(-Optrede_1).toG_I_b())),
      Line5);
  canvas.drawLine(
      Offset((JA + (Antrede_1).toG_I_b() + JK).toMirror(M, MV),
          (TinggiCanvasa - JB+(-Optrede_1).toG_I_b())),
      Offset((JA + (Antrede_1).toG_I_b() + JK).toMirror(M, MV),
          (TinggiCanvasa - JB +(-Optrede_1*2).toG_I_b())),
      Line5);
  canvas.drawLine(
      Offset((JA + (Antrede_1).toG_I_b() + JK).toMirror(M, MV),
          (TinggiCanvasa - JB+(-Optrede_1*2).toG_I_b())),
      Offset((JA + (Antrede_1*2).toG_I_b() + JK).toMirror(M, MV),
          (TinggiCanvasa - JB +(-Optrede_1*2).toG_I_b())),
      Line5);
  canvas.drawLine(
      Offset((JA + (Antrede_1*2.5).toG_I_b() + JK).toMirror(M, MV),
          (TinggiCanvasa - JB/2+(0.00).toG_I_b())),
      Offset((JA + (Antrede_1*2.5).toG_I_b() + JK).toMirror(M, MV),
          (TinggiCanvasa - JB/2 +(-Optrede_1).toG_I_b())),
      Line5);
  canvas.drawLine(
      Offset(((JA + (Antrede_1*2.5).toG_I_b() + JK).toMirror(M, MV)),
          (TinggiCanvasa - JB/2+(-Optrede_1).toG_I_b())),
      Offset((JA + (Antrede_1*3.5).toG_I_b() + JK).toMirror(M, MV),
          (TinggiCanvasa - JB/2 +(-Optrede_1).toG_I_b())),
      Line5);
  double miringy =TinggiMiringAnakTangga_1*cosa_1;
  double miringx =TinggiMiringAnakTangga_1*sina_1;
  ...
  double tebalsx =15*sina_1;
  canvas.drawLine(
      Offset((JA + (Antrede_1*2+tebalsx).toG_I_b() + JK).toMirror(M, MV),
          (TinggiCanvasa - JB+(-Optrede_1*2+tebalsy).toG_I_b())),
      Offset((JA + (Antrede_1*2-miringx).toG_I_b() + JK).toMirror(M, MV),
          (TinggiCanvasa - JB +(-Optrede_1*2-miringy).toG_I_b())),
      Line6);
  canvas.drawLine(
      Offset((JA + (tebalsx).toG_I_b() + JK).toMirror(M, MV),
          (TinggiCanvasa - JB+(tebalsy).toG_I_b())),
      Offset((JA + (-miringx).toG_I_b() + JK).toMirror(M, MV),
          (TinggiCanvasa - JB +(-miringy).toG_I_b())),
      Line6);
  canvas.drawLine(
      Offset((JA + (-miringx).toG_I_b() + JK).toMirror(M, MV),
          (TinggiCanvasa - JB+(-miringy).toG_I_b())),
      ...
      Line6);
  canvas.drawLine(
      Offset((JA + (-miringx/2).toG_I_b() + JK).toMirror(M, MV),
          (TinggiCanvasa - JB+(-miringy/2).toG_I_b())),
      Offset((JA + (Antrede_1*2-miringx/2).toG_I_b() + JK).toMirror(M, MV),
          (TinggiCanvasa - JB +(-Optrede_1*2-miringy/2).toG_I_b())),
      Line6);
  canvas.drawLine(
      Offset((JA + (0.0).toG_I_b() + JK).toMirror(M, MV),
          (TinggiCanvasa - JB+(0.0).toG_I_b())),
      Offset((JA + (Antrede_1*2).toG_I_b() + JK).toMirror(M, MV),
          (TinggiCanvasa - JB +(-Optrede_1*2).toG_I_b())),
      Line6);
  canvas.drawLine(
  ...
  //keterangan
  //keterangan Antrede
  final Kete = Para(Text:Antrede_1.toStringAsFixed(0) + " cm",Lebar: Antrede_1.toG_I_b(),Posisi: TextAlign.center,font: 7);
  canvas.drawParagraph(Kete.toKetB(),
      Offset((JA+JK+(Antrede_1*2.5).toG_I_b()).toMirror(M, MV) , (TinggiCanvasa - JB/2-JK +(-Optrede_1).toG_I_b())));
  //keterangan1 Optrede
  final Kete1 = Para(Text:Optrede_1.toStringAsFixed(0) + " cm",Lebar: Optrede_1.toG_I_b(),font: 7);
  canvas.drawParagraph(Kete1.toKetB(),
      Offset((JA+JK+(Antrede_1*1.7).toG_I_b()).toMirror(M, MV) , (TinggiCanvasa - JB/2+(-Optrede_1/2).toG_I_b())));
//keterangan2 0.5 tebal plat tangga
  final Kete2 = Para(Text:TinggiMiring5AnakTangga_1.toStringAsFixed(1) + " cm",font: 7);
  canvas.drawParagraph(Kete2.toKetB(),
      Offset((JA+JK/2+(Antrede_1*2).toG_I_b()).toMirror(M, MV) , (TinggiCanvasa - JB-JK*1.5+(-Optrede_1*2).toG_I_b())));
//keterangan3 Tebal pelat tangga pilih
  final Kete3 = Para(Text:(TebalPelatTanggaPilih_1/10).toStringAsFixed(1)+" cm",font: 7);
  canvas.drawParagraph(Kete3.toKetB(),
      Offset((JA+JK*1.5+(Antrede_1*2).toG_I_b()).toMirror(M, MV) , (TinggiCanvasa - JB-JK+(-Optrede_1*1.5).toG_I_b())));
...
void generatePdfGambar3_I_1B_3() async {
  final int r =4;
  final int x=400*r;
  final int y=250*r;
  final recorder = ui.PictureRecorder();
  ...
  canvas.drawLine(
      Offset((JA+(0.0).toG_I_c()).toMirror(M, MV), TinggiCanvasa - JB+(0.0).toG_I_c()),
      Offset((JA+(PanjangT_1).toG_I_c()).toMirror(M, MV), TinggiCanvasa - JB+(-TinggiT_1).toG_I_c()),
      Line5);

  canvas.drawLine(
      Offset((JA+(0.0).toG_I_c()).toMirror(M, MV), TinggiCanvasa - JB-gaya+(0.0).toG_I_c()),
      Offset((JA+(PanjangT_1).toG_I_c()).toMirror(M, MV), TinggiCanvasa - JB-gaya+(-TinggiT_1).toG_I_c()),
      Line5);
  for (var i = 0; i <= (PanjangT_1/20).toRound(0); i++) {
    canvas.drawLine(
        Offset((JA+(PanjangT_1/(PanjangT_1/20).toRound(0)*i).toG_I_c()).toMirror(M, MV),
            TinggiCanvasa - JB+(-TinggiT_1/(PanjangT_1/20).toRound(0)*i).toG_I_c()),
        Offset((JA+(PanjangT_1/(PanjangT_1/20).toRound(0)*i).toG_I_c()).toMirror(M, MV),
            TinggiCanvasa - JB-gaya+(-TinggiT_1/(PanjangT_1/20).toRound(0)*i).toG_I_c()),
        Line5);
    canvas.drawLine(
        Offset((JA+(PanjangT_1/(PanjangT_1/20).toRound(0)*i).toG_I_c()).toMirror(M, MV),
            TinggiCanvasa - JB+(-TinggiT_1/(PanjangT_1/20).toRound(0)*i).toG_I_c()),
        Offset((JA+5+(PanjangT_1/(PanjangT_1/20).toRound(0)*i).toG_I_c()).toMirror(M, MV),
            TinggiCanvasa - JB-5+(-TinggiT_1/(PanjangT_1/20).toRound(0)*i).toG_I_c()),
        Line5);
    canvas.drawLine(
        Offset((JA+(PanjangT_1/(PanjangT_1/20).toRound(0)*i).toG_I_c()).toMirror(M, MV),
            TinggiCanvasa - JB+(-TinggiT_1/(PanjangT_1/20).toRound(0)*i).toG_I_c()),
  ...
  final arc1 = Path();
  arc1.moveTo((JA+(PanjangT_1*4/5).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+20+(-TinggiT_1*4/5).toG_I_c());
  arc1.arcToPoint(
      Offset((JA+20*sina_1+(PanjangT_1*4/5).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+20+20*cosa_1+(-TinggiT_1*4/5).toG_I_c()),
      radius: Radius.circular(2),
      clockwise: true
  );
  ...
  canvas.drawLine(
      Offset((JA+20*sina_1+(PanjangT_1*4/5).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+20+20*cosa_1+(-TinggiT_1*4/5).toG_I_c()),
      Offset((JA+20*sina_1+(PanjangT_1*4/5).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+20+20*cosa_1-5+(-TinggiT_1*4/5).toG_I_c()),
      Line5);
  canvas.drawLine(
      Offset((JA+20*sina_1+(PanjangT_1*4/5).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+20+20*cosa_1+(-TinggiT_1*4/5).toG_I_c()),
      Offset((JA+20*sina_1+5+(PanjangT_1*4/5).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+20+20*cosa_1+(-TinggiT_1*4/5).toG_I_c()),
      Line5);

  //bawah
  arc1.moveTo((JA+(PanjangT_1*3/10).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+20+(-TinggiT_1*3/10).toG_I_c());
  arc1.arcToPoint(
      Offset((JA+20*sina_1+(PanjangT_1*3/10).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+20+20*cosa_1+(-TinggiT_1*3/10).toG_I_c()),
      radius: Radius.circular(2),
      clockwise: false
  );
  canvas.drawPath(arc1, Line7);
  canvas.drawLine(
      Offset((JA+20*sina_1+(PanjangT_1*3/10).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+20+20*cosa_1+(-TinggiT_1*3/10).toG_I_c()),
      Offset((JA+20*sina_1-4.5+(PanjangT_1*3/10).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+20+20*cosa_1+4.5+(-TinggiT_1*3/10).toG_I_c()),
      Line5);
  canvas.drawLine(
      Offset((JA+20*sina_1+(PanjangT_1*3/10).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+20+20*cosa_1+(-TinggiT_1*3/10).toG_I_c()),
      Offset((JA+20*sina_1-4.5+(PanjangT_1*3/10).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+20+20*cosa_1-4.5+(-TinggiT_1*3/10).toG_I_c()),
...
//garis gaya //AB //H
  canvas.drawLine(
      Offset((JA-JK+(0.0).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+(0.0).toG_I_c()),
      Offset((JA-JK-20+(0.0).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+(0.0).toG_I_c()),
      Line5);
  canvas.drawLine(
      Offset((JA-JK+(0.0).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+(0.0).toG_I_c()),
      Offset((JA-JK-5+(0.0).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+5+(0.0).toG_I_c()),
      Line5);
  canvas.drawLine(
      Offset((JA-JK+(0.0).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+(0.0).toG_I_c()),
      Offset((JA-JK-5+(0.0).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB-5+(0.0).toG_I_c()),
  ...
  //V
  canvas.drawLine(
      Offset((JA+(0.0).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+JK+(0.0).toG_I_c()),
      Offset((JA+(0.0).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+JK+20+(0.0).toG_I_c()),
      Line5);
  canvas.drawLine(
      Offset((JA+(0.0).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+JK+(0.0).toG_I_c()),
      Offset((JA-5+(0.0).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+JK+5+(0.0).toG_I_c()),
      Line5);
  canvas.drawLine(
      Offset((JA+(0.0).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+JK+(0.0).toG_I_c()),
      Offset((JA+5+(0.0).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+JK+5+(0.0).toG_I_c()),
  ...
  //BA //H
  canvas.drawLine(
      Offset((JA+JK+(PanjangT_1).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+(-TinggiT_1).toG_I_c()),
      Offset((JA+JK+20+(PanjangT_1).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+(-TinggiT_1).toG_I_c()),
      Line5);
  canvas.drawLine(
      Offset((JA+JK+(PanjangT_1).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+(-TinggiT_1).toG_I_c()),
      Offset((JA+JK+5+(PanjangT_1).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+5+(-TinggiT_1).toG_I_c()),
      Line5);
  canvas.drawLine(
      Offset((JA+JK+(PanjangT_1).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+(-TinggiT_1).toG_I_c()),
      Offset((JA+JK+5+(PanjangT_1).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB-5+(-TinggiT_1).toG_I_c()),
  ...
  //V
  canvas.drawLine(
      Offset((JA+(PanjangT_1).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+JK+(-TinggiT_1).toG_I_c()),
      Offset((JA+(PanjangT_1).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+JK+20+(-TinggiT_1).toG_I_c()),
      Line5);
  canvas.drawLine(
      Offset((JA+(PanjangT_1).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+JK+(-TinggiT_1).toG_I_c()),
      Offset((JA-5+(PanjangT_1).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+JK+5+(-TinggiT_1).toG_I_c()),
      Line5);
  canvas.drawLine(
      Offset((JA+(PanjangT_1).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+JK+(-TinggiT_1).toG_I_c()),
      Offset((JA+5+(PanjangT_1).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+JK+5+(-TinggiT_1).toG_I_c()),
  ...
  canvas.drawParagraph(Kete.toKetB(),
      Offset((JA-JK-10+(0.0).toG_I_c()).toMirror(M, MV) , (TinggiCanvasa - JB +10+(0.0).toG_I_c())));
  //keterangan B
  final Kete1 = Para(Text:"B",font: 12);
  canvas.drawParagraph(Kete1.toKetB(),
      Offset((JA+JK+(PanjangT_1).toG_I_c()).toMirror(M, MV) , (TinggiCanvasa - JB +10+(-TinggiT_1).toG_I_c())));
  //keterangan MaAB
  final Kete2 = Para(Text:MaAB.toStringAsFixed(1) + " kNm",font: 10);
  canvas.drawParagraph(Kete2.toKetB(),
      Offset((JA-20+(PanjangT_1*3/10).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+20+20+(-TinggiT_1*3/10).toG_I_c()));
//keterangan MaBA
  final Kete3 = Para(Text:MaBA.toStringAsFixed(1) + " kNm",font: 10);
  canvas.drawParagraph(Kete3.toKetB(),
      Offset((JA+20+(PanjangT_1*4/5).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+20+5+(-TinggiT_1*4/5).toG_I_c()));
//keterangan beban terfaktor
  final Kete4 = Para(Text:BebanTerfaktorTangga_1.toStringAsFixed(1) + " kN/m",font: 10);
  canvas.drawParagraph(Kete4.toKetB(),
      Offset((JA-40+(PanjangT_1/2).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB-gaya-30+(-TinggiT_1/2).toG_I_c()));
//keterangan beban FHAB
  final Kete5 = Para(Text:FHbAB.toStringAsFixed(1) + " kN",font: 10);
  canvas.drawParagraph(Kete5.toKetB(),
      Offset((JA-JK*5+(0.0).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB-JK*2+(0.0).toG_I_c()));
//keterangan beban FVAB
  final Kete6 = Para(Text:FVbAB.toStringAsFixed(1) + " kN",font: 10);
  canvas.drawParagraph(Kete6.toKetB(),
      Offset((JA+JK+(0.0).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+JK*2+(0.0).toG_I_c()));
//keterangan beban FHBA
  final Kete7 = Para(Text:FHbAB.toStringAsFixed(1) + " kN",font: 10);
  canvas.drawParagraph(Kete7.toKetB(),
      Offset((JA+JK*2+(PanjangT_1).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB-JK*2+(-TinggiT_1).toG_I_c()));
//keterangan beban FVBA
  final Kete8 = Para(Text:FVbAB.toStringAsFixed(1) + " kN",font: 10);
  canvas.drawParagraph(Kete8.toKetB(),
      Offset((JA+JK+(PanjangT_1).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+JK*3+(-TinggiT_1).toG_I_c()));

  //
  // var path = Path();
  //
  // path.moveTo((JA+(0.0).toG_I_c()).toMirror(M, MV), TinggiCanvasa - JB+(0.0).toG_I_c());
  // path.lineTo((JA+(0.0).toG_I_c()).toMirror(M, MV), TinggiCanvasa - JB-gaya+(0.0).toG_I_c());
  // path.lineTo((JA+(PanjangT_1).toG_I_c()).toMirror(M, MV), TinggiCanvasa - JB-gaya+(-TinggiT_1).toG_I_c());
  // path.lineTo((JA+(PanjangT_1).toG_I_c()).toMirror(M, MV), TinggiCanvasa - JB+(-TinggiT_1).toG_I_c());
  // canvas.drawPath(path, Line7);
  // canvas.drawLine(
  //     Offset(0,0),
...
void generatePdfGambar4_I_1B_4() async {
  final int r =4;
  final int x=400*r;
  final int y=250*r;
  final recorder = ui.PictureRecorder();
  ...
  canvas.drawLine(
      ...
      Line5);
  canvas.drawLine(
      Offset((JA-gaya*cs+(0.0).toG_I_c()).toMirror(M, MV), TinggiCanvasa - JB-gaya*cc+(0.0).toG_I_c()),
      Offset((JA-gaya*cs+(PanjangT_1).toG_I_c()).toMirror(M, MV), TinggiCanvasa - JB-gaya*cc+(-TinggiT_1).toG_I_c()),
      Line5);
  for (var i = 0; i <= (PanjangT_1/20).toRound(0); i++) {
    canvas.drawLine(
        Offset((JA+(PanjangT_1/(PanjangT_1/20).toRound(0)*i).toG_I_c()).toMirror(M, MV),
            TinggiCanvasa - JB+(-TinggiT_1/(PanjangT_1/20).toRound(0)*i).toG_I_c()),
        Offset((JA-gaya*cs+(PanjangT_1/(PanjangT_1/20).toRound(0)*i).toG_I_c()).toMirror(M, MV),
            TinggiCanvasa - JB-gaya*cc+(-TinggiT_1/(PanjangT_1/20).toRound(0)*i).toG_I_c()),
        Line5);
    canvas.drawLine(
        Offset((JA+(PanjangT_1/(PanjangT_1/20).toRound(0)*i).toG_I_c()).toMirror(M, MV),
            TinggiCanvasa - JB+(-TinggiT_1/(PanjangT_1/20).toRound(0)*i).toG_I_c()),
        Offset((JA-(anak*cs)+(anak*cc)+(PanjangT_1/(PanjangT_1/20).toRound(0)*i).toG_I_c()).toMirror(M, MV),
            TinggiCanvasa - JB-anak*cs-cc*anak+(-TinggiT_1/(PanjangT_1/20).toRound(0)*i).toG_I_c()),
        Line5);
    canvas.drawLine(
        Offset((JA+(PanjangT_1/(PanjangT_1/20).toRound(0)*i).toG_I_c()).toMirror(M, MV),
            TinggiCanvasa - JB+(-TinggiT_1/(PanjangT_1/20).toRound(0)*i).toG_I_c()),
        Offset((JA-anak*cs-cc*anak+(PanjangT_1/(PanjangT_1/20).toRound(0)*i).toG_I_c()).toMirror(M, MV),
            TinggiCanvasa - JB+(anak*cs)-(anak*cc)+(-TinggiT_1/(PanjangT_1/20).toRound(0)*i).toG_I_c()),
        Line5);
  }
  //panah melengkung atas
  final arc1 = Path();
  arc1.moveTo((JA+(PanjangT_1*4/5).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+20+(-TinggiT_1*4/5).toG_I_c());
  arc1.arcToPoint(
      Offset((JA+20*sina_1+(PanjangT_1*4/5).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+20+20*cosa_1+(-TinggiT_1*4/5).toG_I_c()),
...
//garis gaya //AB //H
  canvas.drawLine(
      Offset((JA-JK+(0.0).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+(0.0).toG_I_c()),
      Offset((JA-JK-panah*cc+(0.0).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+panah*cs+(0.0).toG_I_c()),
      Line5);
  canvas.drawLine(
      Offset((JA-JK+(0.0).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+(0.0).toG_I_c()),
      Offset((JA-JK-anak*cs-anak*cc+(0.0).toG_I_c()).toMirror(M, MV),
          TinggiCanvasa-JB+anak*cs-anak*cc+(0.0).toG_I_c()),
      Line5);
  canvas.drawLine(
      Offset((JA-JK+(0.0).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+(0.0).toG_I_c()),
      Offset((JA-JK+anak*cs-anak*cc+(0.0).toG_I_c()).toMirror(M, MV),
  ...
  canvas.drawLine(
      Offset((JA+(0.0).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+JK+(0.0).toG_I_c()),
      Offset((JA+panah*cs+(0.0).toG_I_c()).toMirror(M, MV),
          TinggiCanvasa-JB+JK+panah*cc+(0.0).toG_I_c()),
      Line5);
  canvas.drawLine(
      Offset((JA+(0.0).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+JK+(0.0).toG_I_c()),
      Offset((JA+anak*cs+anak*cc+(0.0).toG_I_c()).toMirror(M, MV),
          TinggiCanvasa-JB+JK-anak*cs+anak*cc+(0.0).toG_I_c()),
      Line5);
  canvas.drawLine(
      Offset((JA+(0.0).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+JK+(0.0).toG_I_c()),
      Offset((JA+anak*cs-anak*cc+(0.0).toG_I_c()).toMirror(M, MV),
  ...
  canvas.drawLine(
      Offset((JA+JK+(PanjangT_1).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+(-TinggiT_1).toG_I_c()),
      Offset((JA+JK+panah*cc+(PanjangT_1).toG_I_c()).toMirror(M, MV),
          TinggiCanvasa-JB-panah*cs+(-TinggiT_1).toG_I_c()),
      Line5);
  canvas.drawLine(
      Offset((JA+JK+(PanjangT_1).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+(-TinggiT_1).toG_I_c()),
      Offset((JA+JK+anak*cs+anak*cc+(PanjangT_1).toG_I_c()).toMirror(M, MV),
          TinggiCanvasa-JB-anak*cs+anak*cc+(-TinggiT_1).toG_I_c()),
      Line5);
  canvas.drawLine(
      Offset((JA+JK+(PanjangT_1).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+(-TinggiT_1).toG_I_c()),
      Offset((JA+JK-anak*cs+anak*cc+(PanjangT_1).toG_I_c()).toMirror(M, MV),
  ...
  canvas.drawLine(
      Offset((JA+(PanjangT_1).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+JK+(-TinggiT_1).toG_I_c()),
      Offset((JA+panah*cs+(PanjangT_1).toG_I_c()).toMirror(M, MV),
          TinggiCanvasa-JB+JK+panah*cc+(-TinggiT_1).toG_I_c()),
      Line5);
  canvas.drawLine(
      Offset((JA+(PanjangT_1).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+JK+(-TinggiT_1).toG_I_c()),
      Offset((JA+anak*cs-anak*cc+(PanjangT_1).toG_I_c()).toMirror(M, MV),
          TinggiCanvasa-JB+JK+anak*cs+anak*cc+(-TinggiT_1).toG_I_c()),
      Line5);
  canvas.drawLine(
      Offset((JA+(PanjangT_1).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+JK+(-TinggiT_1).toG_I_c()),
      Offset((JA+anak*cs+anak*cc+(PanjangT_1).toG_I_c()).toMirror(M, MV),
...
//keterangan MaBA
  ...
  canvas.drawParagraph(Kete3.toKetB(),
      Offset((JA+20+(PanjangT_1*4/5).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+20+5+(-TinggiT_1*4/5).toG_I_c()));
//keterangan beban terfaktor
  final Kete4 = Para(Text:(BebanTerfaktorTangga_1*cosa_1).toStringAsFixed(1) + " kN/m",font: 10);
  canvas.drawParagraph(Kete4.toKetB(),
      Offset((JA-40+(PanjangT_1/2).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB-gaya-30+(-TinggiT_1/2).toG_I_c()));
//keterangan beban NAB
  final Kete5 = Para(Text:NAB.toStringAsFixed(1) + " kN",font: 10);
  canvas.drawParagraph(Kete5.toKetB(),
      Offset((JA-JK*5+(0.0).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB-JK*2+(0.0).toG_I_c()));
//keterangan beban SAB
  final Kete6 = Para(Text:SAB.toStringAsFixed(1) + " kN",font: 10);
  canvas.drawParagraph(Kete6.toKetB(),
      Offset((JA+JK+(0.0).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+JK*2+(0.0).toG_I_c()));
//keterangan beban NBA
  final Kete7 = Para(Text:NBA.toStringAsFixed(1) + " kN",font: 10);
  canvas.drawParagraph(Kete7.toKetB(),
      Offset((JA+JK*2+(PanjangT_1).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB-JK*2+(-TinggiT_1).toG_I_c()));
//keterangan beban SBA
  final Kete8 = Para(Text:SBA.toStringAsFixed(1) + " kN",font: 10);
  canvas.drawParagraph(Kete8.toKetB(),
      Offset((JA+JK+(PanjangT_1).toG_I_c()).toMirror(M, MV),TinggiCanvasa-JB+JK*3+(-TinggiT_1).toG_I_c()));
...
void generatePdfGambar5_I_1B_5() async {
  final int r =4;
  final int x=400*r;
  final int y=250*r;
  final recorder = ui.PictureRecorder();
  ...
  canvas.drawLine(
      Offset((JA+(0.0).toG_I_c()).toMirror(M, MV),
          TinggiCanvasa - JB+(0.0).toG_I_c()),
      Offset((JA+(PanjangT_1).toG_I_c()).toMirror(M, MV),
          TinggiCanvasa - JB+(-TinggiT_1).toG_I_c()),
      Line5);
  canvas.drawLine(
      Offset((JA+(0.0).toG_I_c()).toMirror(M, MV),
          TinggiCanvasa - JB+(0.0).toG_I_c()),
      Offset((JA+(-NAB*4*cs).toG_I_c()).toMirror(M, MV),
          TinggiCanvasa - JB+(-NAB*cc*4).toG_I_c()),
      Line5);
  canvas.drawLine(
      Offset((JA+(PanjangT_1).toG_I_c()).toMirror(M, MV),
          TinggiCanvasa - JB+(-TinggiT_1).toG_I_c()),
      Offset((JA+(PanjangT_1-NBA*4*cs).toG_I_c()).toMirror(M, MV),
          TinggiCanvasa - JB+(-TinggiT_1-NBA*4*cc).toG_I_c()),
      Line5);
  canvas.drawLine(
      Offset((JA+(-NAB*4*cs).toG_I_c()).toMirror(M, MV),
          TinggiCanvasa - JB+(-NAB*cc*4).toG_I_c()),
...
// keterangan NAB
  ...
  canvas.drawParagraph(Kete4.toKetB(),
      Offset((JA-JK*3+(-NAB*4*cs).toG_I_c()).toMirror(M, MV),
          TinggiCanvasa - JB-JK*2+(-NAB*cc*4).toG_I_c()));
// keterangan NBA
  final Kete1 = Para(Text:(NBA).toStringAsFixed(1) + " kN/m",font: 10);
  canvas.drawParagraph(Kete1.toKetB(),
      Offset((JA-JK*5+(PanjangT_1-NBA*4*cs).toG_I_c()).toMirror(M, MV),
          TinggiCanvasa - JB+(-TinggiT_1-NBA*4*cc).toG_I_c()));
...
void generatePdfGambar6_I_1B_6() async {
  final int r =4;
  final int x=400*r;
  final int y=250*r;
  final recorder = ui.PictureRecorder();
  ...
  canvas.drawLine(
      Offset((JA+(0.0).toG_I_c()).toMirror(M, MV),
          TinggiCanvasa - JB+(0.0).toG_I_c()),
      Offset((JA+(-SAB*4*cs).toG_I_c()).toMirror(M, MV),
          TinggiCanvasa - JB+(-SAB*cc*4).toG_I_c()),
      Line5);
  canvas.drawLine(
      Offset((JA+(PanjangT_1).toG_I_c()).toMirror(M, MV),
          TinggiCanvasa - JB+(-TinggiT_1).toG_I_c()),
      Offset((JA+(PanjangT_1+SBA*4*cs).toG_I_c()).toMirror(M, MV),
          TinggiCanvasa - JB+(-TinggiT_1+SBA*4*cc).toG_I_c()),
      Line5);
  canvas.drawLine(
      Offset((JA+(-SAB*4*cs).toG_I_c()).toMirror(M, MV),
          TinggiCanvasa - JB+(-SAB*cc*4).toG_I_c()),
...
// keterangan NAB
  ...
  canvas.drawParagraph(Kete4.toKetB(),
      Offset((JA-JK*3+(-SAB*4*cs).toG_I_c()).toMirror(M, MV),
          TinggiCanvasa - JB-JK*2+(-SAB*cc*4).toG_I_c()));
// keterangan NBA
  final Kete1 = Para(Text:(SBA).toStringAsFixed(1) + " kN/m",font: 10);
  canvas.drawParagraph(Kete1.toKetB(),
      Offset((JA-JK*5+(PanjangT_1+SBA*4*cs).toG_I_c()).toMirror(M, MV),
          TinggiCanvasa - JB+JK+(-TinggiT_1+SBA*4*cc).toG_I_c()));
...
void generatePdfGambar7_I_1B_7() async {
  final int r =4;
  final int x=400*r;
  final int y=250*r;
  final recorder = ui.PictureRecorder();
  ...
  canvas.drawLine(
      Offset((JA+(0.0).toG_I_c()).toMirror(M, MV),
          TinggiCanvasa - JB+(0.0).toG_I_c()),
      Offset((JA+(-MAB*4*cs).toG_I_c()).toMirror(M, MV),
          TinggiCanvasa - JB+(-MAB*cc*4).toG_I_c()),
      Line5);
  canvas.drawLine(
      Offset((JA+(PanjangT_1).toG_I_c()).toMirror(M, MV),
          TinggiCanvasa - JB+(-TinggiT_1).toG_I_c()),
      Offset((JA+(PanjangT_1+MBA*4*cs).toG_I_c()).toMirror(M, MV),
          TinggiCanvasa - JB+(-TinggiT_1+MBA*4*cc).toG_I_c()),
      Line5);

  var arc = Path();
  arc.moveTo(
    //kiri
      (JA+(-MAB*4*cs).toG_I_c()).toMirror(M, MV),
      TinggiCanvasa - JB+(-MAB*cc*4).toG_I_c());
  arc.quadraticBezierTo(
    //tengah
      (JA+(JAB*100*cosa_1-(MLAB-MAB)*4*sina_1).toG_I_c()).toMirror(M, MV),
      TinggiCanvasa - JB+(-JAB*100*sina_1-(MLAB-MAB)*4*cosa_1).toG_I_c(),
      //kanan
      (JA+(PanjangT_1+MBA*4*cs).toG_I_c()).toMirror(M, MV),
      TinggiCanvasa - JB+(-TinggiT_1+MBA*4*cc).toG_I_c());
...
// keterangan MAB
  ...
  canvas.drawParagraph(Kete4.toKetB(),
      Offset((JA-JK*3+(-MAB*4*cs).toG_I_c()).toMirror(M, MV),
          TinggiCanvasa - JB-JK+(-SAB*cc*4).toG_I_c()));
// keterangan MBA
  final Kete1 = Para(Text:(MBA).toStringAsFixed(1) + " kNm",font: 10);
  canvas.drawParagraph(Kete1.toKetB(),
      Offset((JA-JK*5+(PanjangT_1+MBA*4*cs).toG_I_c()).toMirror(M, MV),
          TinggiCanvasa - JB-JK+(-TinggiT_1).toG_I_c()));
// keterangan MLAB
  final Kete2 = Para(Text:(MLAB).toStringAsFixed(1) + " kN/m",font: 10);
  canvas.drawParagraph(Kete2.toKetB(),
      Offset( (JA+(JAB*100*cosa_1-(MLAB-MAB)*2*sina_1).toG_I_c()).toMirror(M, MV),
        TinggiCanvasa - JB+(-JAB*100*sina_1-(MLAB-MAB)*2*cosa_1).toG_I_c(),));
...
void generatePdfGambar_I_1B() async{
  final recorder = ui.PictureRecorder();
  ...
  generatePdfGambar1_I_1B_1();
  generatePdfGambar2_I_1B_2();
  generatePdfGambar3_I_1B_3();
  generatePdfGambar4_I_1B_4();
  generatePdfGambar5_I_1B_5();
  generatePdfGambar6_I_1B_6();
  generatePdfGambar7_I_1B_7();
}